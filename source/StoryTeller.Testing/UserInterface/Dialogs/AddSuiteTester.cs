...
namespace StoryTeller.Testing.UserInterface.Dialogs
{
    [TestFixture]
    public class AddSuiteTester : InteractionContext<AddSuite>
    {
        [Test]
        ...
        {
            string theSuiteNameEnteredByTheUser = "something";
            ClassUnderTest.NameBox.Text = theSuiteNameEnteredByTheUser;
            ControlDriver.FireEvent(ClassUnderTest.Create, "Click");

            MockFor<IAddSuiteCommand>().AssertWasCalled(x => x.CreateSuite(theSuiteNameEnteredByTheUser));
        }
