...
{
    [TestFixture]
    public class MarkAcceptanceCommandTester : InteractionContext<MarkAcceptanceCommand>
    {
...
");
        ...
        public void should_have_saved_the_changed_tests()
        {
            ...
            service.AssertWasCalled(x => x.Save(tests.First(o => o.Name == "t4")));
            service.AssertWasCalled(x => x.Save(tests.First(o => o.Name == "t5")));
            service.AssertWasCalled(x => x.Save(tests.First(o => o.Name == "t6")));
        }

        [Test]
        public void should_have_toggled_the_lifecyle_of_all_regression_tests_to_acceptance()
        {
            tests.Each(x => x.Lifecycle.ShouldEqual(Lifecycle.Acceptance));
        }
