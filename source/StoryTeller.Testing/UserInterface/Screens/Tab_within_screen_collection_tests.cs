...
namespace StoryTeller.Testing.UserInterface.Screens
{
    [TestFixture]
    public class Tab_within_screen_collection_tests
    {
        ...
        {
            tab.Header.As<StackPanel>().Children[0].ShouldBeOfType<Label>().Content.ShouldEqual(screen.Title);
        }

        [Test]
        public void the_view_is_added_to_the_content_panel_of_the_tab_item()
        {
            tab.Content.As<DockPanel>().Children[0].ShouldBeTheSameAs(screen.View);
        }
    }
